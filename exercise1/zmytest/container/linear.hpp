...
template <typename Data>
void TestSort(unsigned int &testnum, unsigned int &testerr,
     lasd::SortableLinearContainer<Data> &con) {
  bool tst = true;
  testnum++;
  try {
  std::cout << " " << testnum << " (" << testerr
    << ") Sorting the linear container: ";
  con.Sort();
  if (!con.Empty()){
    for (unsigned long i = 0; i < con.Size() - 1; i++)
    {
      if (con[i] > con[i + 1])
      {
      tst = false;

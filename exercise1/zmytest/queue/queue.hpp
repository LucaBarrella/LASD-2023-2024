...
template <typename Data>
void Clear(unsigned int &testnum, unsigned int &testerr, lasd::Queue<Data> &que, bool chk) {
  testnum++;
  bool tst = true;
  try {
    std::cout << " " << testnum << " (" << testerr << ") Clear the queue: ";
    que.Clear();
    std::cout << "Correct!" << std::endl;
  } catch (std::exception &exc) {
    std::cout << "\"" << exc.what() << "\": " << "Error!" << std::endl;
...
//TODO Vedere se qualche altro metodo può essere coperto
